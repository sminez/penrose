...
//!
//! Variants carry the keysym names used by `xmodmap` and `XKeysymToString` with
//! the `XK_` / `XF86XK_` prefix stripped, so [`XKeySym`]'s `FromStr` impl parses
//! the key names written in binding strings, and [`XKeySym::keysym`] gives the
//! value they denote.
use strum::*;
...
impl XKeySym {
    /// The X11 keysym value for this key.
    ///
    /// Keysym numbering is shared with xkbcommon, so this is also the value a
    /// Wayland compositor expects when binding a key.
...
mod tests {
    ...
    #[test]
    fn keysym_values_are_correct() {
        // Spot checks across the ranges: latin1 is the codepoint itself, the
        // function keys live in 0xff00, media keys in 0x1008ff00.
        assert_eq!(XKeySym::XK_a.keysym(), 0x0061);
        assert_eq!(XKeySym::XK_A.keysym(), 0x0041);
        assert_eq!(XKeySym::XK_semicolon.keysym(), 0x003b);
        assert_eq!(XKeySym::XK_Return.keysym(), 0xff0d);
        assert_eq!(XKeySym::XK_Escape.keysym(), 0xff1b);
        assert_eq!(XKeySym::XF86XK_AudioMute.keysym(), 0x1008ff12);
    }

    #[test]
    fn names_parse_to_keysyms() {
        // The names as they are written in binding strings, which are also the
        // names xmodmap -pke prints.
        for (name, expected) in [
            ...
            ("XF86AudioMicMute", 0x1008ff18),
        ] {
            let sym = XKeySym::from_str(name).expect(name);
            assert_eq!(sym.keysym(), expected, "{name}");
        }
    }

    #[test]
    fn every_keysym_is_non_zero() {
        // A zero would mean a missing arm and NoSymbol is not in the enum, so
        // this catches a botched regeneration.
        for sym in XKeySym::iter() {
            assert_ne!(sym.keysym(), 0, "{sym:?}");
        }
    }

    #[test]
    fn utf8_conversion_still_works() {
        assert_eq!(XKeySym::XK_a.as_utf8_string().unwrap(), "a");
        assert_eq!(XKeySym::XK_A.as_utf8_string().unwrap(), "A");
    }
}
